...
import { Search, Plus, User, Phone, Mail, Edit2, X, Trash2 } from "lucide-react";
...
export default function CRMView() {
  ...
  // 🔥 NUEVA FUNCIÓN: ELIMINAR CLIENTE CON CONFIRMACIÓN 🔥
  const handleEliminar = async (id: number, nombre: string) => {
    const confirmar = window.confirm(`¿Estás seguro de que deseas eliminar a "${nombre}" de tu base de clientes?`);
        ...
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full text-left border-collapse min-w-[600px]">
            ...
            <tbody className="divide-y divide-stone-100">
              {isLoading ? (
                <tr><td colSpan={4} className="p-8 text-center text-stone-400 font-medium">Cargando base de datos...</td></tr>
              ) : clientesFiltrados.length === 0 ? (
                <tr><td colSpan={4} className="p-8 text-center text-stone-400 font-medium">No se encontraron clientes.</td></tr>
              ) : (
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6 relative">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input required type="text" value={formData.nombre} onChange={e => setFormData({...formData, nombre: e.target.value})} className="w-full border-2 border-stone-200 rounded-lg p-2.5 outline-none focus:border-emerald-500 text-sm" placeholder="Ej. Juan Pérez" />
              </div>
              <div>
                <label className="block text-[10px] font-bold text-stone-500 uppercase tracking-widest mb-1.5">Celular / WhatsApp</label>
                <input type="tel" value={formData.celular} onChange={e => setFormData({...formData, celular: e.target.value})} className="w-full border-2 border-stone-200 rounded-lg p-2.5 outline-none focus:border-emerald-500 text-sm" placeholder="Ej. 987654321" />
              </div>
              <div>
                ...
                <input type="email" value={formData.correo} onChange={e => setFormData({...formData, correo: e.target.value})} className="w-full border-2 border-stone-200 rounded-lg p-2.5 outline-none focus:border-emerald-500 text-sm" placeholder="Ej. juan@gmail.com" />
              </div>

              <button type="submit" disabled={isSubmitting} className="w-full py-3 mt-4 rounded-lg font-bold text-sm bg-black text-white hover:bg-stone-800 transition-colors disabled:opacity-50">
