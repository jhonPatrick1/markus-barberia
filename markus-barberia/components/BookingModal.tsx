...
export default function BookingModal({ isOpen, preSelection, onClose }: { 
  ...
  const handleSubmit = async (isYape: boolean = false) => {
    ...
    try {
      ...
      // 1. Inserción de la cita en Supabase
      ...
      // =========================================================
      // 🔥 NUEVO: LÓGICA CRM (UPSERT INTELIGENTE) 🔥
      // =========================================================
      try {
        if (selection.celular) {
          const nombreCompleto = `${selection.nombre} ${selection.apellido}`.trim();
          
          // Buscamos si el celular ya existe
          const { data: clienteExistente } = await supabase
            .from('clientes')
            .select('id, cantidad_citas')
            .eq('celular', selection.celular)
            .single();

          if (clienteExistente) {
            // SI EXISTE: Actualizamos sumando 1 cita
            await supabase
              .from('clientes')
              .update({ 
                cantidad_citas: (clienteExistente.cantidad_citas || 0) + 1,
                nombre: nombreCompleto // Actualizamos nombre por si cambió
              })
              .eq('id', clienteExistente.id);
          } else {
            // SI ES NUEVO: Lo insertamos con 1 cita inicial
            await supabase
              .from('clientes')
              .insert({
                nombre: nombreCompleto,
                celular: selection.celular,
                correo: selection.correo || "web@publica.com",
                cantidad_citas: 1 
              });
          }
        }
      } catch (crmError) {
        console.error("Error silencioso al guardar en CRM:", crmError);
      }
      // =========================================================
